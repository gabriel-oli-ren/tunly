      ...
      slivers: [
        ...
        SliverPadding(
          ...
          sliver: SliverList.list(children: [
            ...
            if (hasQuery) ...[
              Text(_searchTerm.isEmpty ? 'Searching…' : 'Results',
                  style: const TextStyle(
                      fontSize: 21, fontWeight: FontWeight.w700)),
              const SizedBox(height: 9),
              if (_searchTerm.length < 2)
                const Text('Keep typing to find songs and artists.',
                    style: TextStyle(
                        color: TunlyTheme.secondaryText, fontSize: 14))
              else
                results!.when(
                  ...
                  data: (tracks) => tracks.isEmpty
                      ? const Padding(
                          ...
                          child: Text('No matches yet. Try another search.',
                              style:
  ...
  Widget build(BuildContext context) => CupertinoButton(
        ...
        child: Row(children: [
          const Icon(CupertinoIcons.clock,
              color: TunlyTheme.secondaryText, size: 18),
          const SizedBox(width: 11),
          Expanded(
              child: Text(value,
                  textAlign: TextAlign.left,
                  style:
                      const TextStyle(color: Color(0xFFF5F5F6), fontSize: 15))),
          const Icon(CupertinoIcons.arrow_up_left,
              color: TunlyTheme.secondaryText, size: 16)
        ]),
      );
}
